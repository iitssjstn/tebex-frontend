...
let r = await (await req("/api/cart", { method: "POST", headers: j, body: JSON.stringify({ action: "add", id: "101", quantity: 2 }) })).json();
ok("add to cart", r.cart?.items?.[0]?.quantity === 2 && r.cart.total === 19.98, JSON.stringify(r));
r = await (await req("/api/cart", { method: "POST", headers: j, body: JSON.stringify({ action: "add", id: "9999" }) })).json();
ok("unknown product refused", !!r.cart?.error);
r = await (await req("/api/cart", { method: "POST", headers: j, body: JSON.stringify({ action: "add", id: "201", quantity: 1, price: 0.01 }) })).json();
ok("client price ignored", r.cart.total === 22.48, r.cart?.total);
let co = await (await req("/api/cart", { method: "POST", headers: j, body: JSON.stringify({ action: "checkout" }) })).json();
ok("checkout asks to sign in first", co.authLinks?.length === 1 && co.authLinks[0].url.startsWith("https://"), JSON.stringify(co));
await fetch(`${MOCK}/__authorize/${jar.sf_basket}`);
co = await (await req("/api/cart", { method: "POST", headers: j, body: JSON.stringify({ action: "checkout" }) })).json();
ok("checkout redirects to Tebex", co.checkoutUrl?.startsWith("https://pay.tebex.example/"), JSON.stringify(co));
